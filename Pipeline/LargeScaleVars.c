...
#define maxlength2 100000   // max read length
#define maxlength1 6000   // max template piece length


//#define PRINT

/*
This tool maps pieces of a repeat template into reads. 
It prints out distance and succession stats and cuts the reads into instances of the repeat.
Those instances are outputted as "SimulatedSeq.fasta".
The information which sequences belong to which read is put into "SimulatedReadSeqInfo".
*/
...
void Help()
{
  printf("Usage: ./LargeScaleVars template.fasta Reads.fasta\n");
  printf("Flags:\n");
...
//This function outputs the information about which sequences are in which read
int seqcount;
...
int main(int argc, char *argv[])
{
  ...
  char *Templatepath_p;
  char *Readspath_p;
  if(argc<2){printf("Usage: ./LargeScaleVars template.fasta Reads.fasta\n");exit(0);}
  Readspath_p=argv[2];
  Templatepath_p=argv[1];

  //the output paths: 
  char outputfile[]="SimulatedSeq.fasta";
  char *output_p=&outputfile[0];
 
  char readseqfile[]="SimulatedReadSeqInfo";
